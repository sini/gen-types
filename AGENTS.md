...
| Search monad, records, either, intensional-function construction; gen-types inlines a two-line name-comparison `typeEq` (`lib/default.nix:47-48`) and takes no gen-algebra input | `gen-algebra` — "gen-algebra: pure Nix algebra — search monad, records, intensional functions, either" |
...
| `override` | present on `struct` results ONLY (`lib/checkers.nix:322`) |
...
| `struct` | `name -> { <member> = checker; } -> checker` with `.override { total ? true, unknown ? true, verify ? null }` |
...
| Export | Signature |
|---|---|
| `typeEq` / `intensionalEq` | `checker -> checker -> bool` (`a.__id == b.__id`; the two are the same function) |
...
| Decide whether two types are the same | `t.typeEq a b` — name-only, see traps |
...
| `struct.override` restarts from DEFAULTS — it does not compose, so chaining silently drops the earlier setting | `lib/checkers.nix:285-322` (`override = build`, and `build` re-reads its own defaults); `(s.override { total = false; }).verify { x = 1; }` ⇒ `null`, but `((s.override { total = false; }).override { unknown = false; }).verify { x = 1; }` ⇒ `"in struct 'point': missing member 'y'"` |
| `unknown` polarity reads backwards: `unknown = true` (the default) PERMITS undeclared keys | `lib/checkers.nix:270-272,319`; `s.verify { x = 1; y = 2; z = 3; }` ⇒ `null`; with `unknown = false` ⇒ `"in struct 'point': keys ['z'] are unrecognized, expected keys are ['x', 'y']"`. Tests: `test-unknown-allowed-by-default`, `test-unknown-rejected-when-closed` (`ci/tests/types-struct.nix`) |
| `__id` hashes `name` alone, so any two types sharing a name are `typeEq` regardless of content. Four collisions observed: `refined` names only the base (`refined<int>`), `strict` names the constant `"strict"`, `enum`/`struct` names are caller-supplied | `lib/checkers.nix:129-131`, `lib/default.nix:47-48`, `lib/refined.nix:45`, `lib/strict.nix:26`; `t.typeEq rp rt` ⇒ `true`; `t.typeEq (t.strict [ "a" ]) (t.strict [ "b" ])` ⇒ `true`; `t.typeEq (t.enum "color" [ "red" ]) (t.enum "color" [ "blue" ])` ⇒ `true`; `t.typeEq (t.struct "p" { x = t.int; }) (t.struct "p" { y = t.str; })` ⇒ `true`. Positive control, same predicate: `t.typeEq (t.listOf t.int) (t.listOf t.str)` ⇒ `false`, `t.typeEq (t.listOf t.int) (t.listOf t.int)` ⇒ `true`. Tests: `test-typeEq-different-names`, `test-typeEq-nested-distinct` (`ci/tests/types-identity.nix`) |
...
| The `optionalAttr` exemption from `total` is POSITIONAL — only a member whose `__name` is exactly `"optionalAttr"`; any wrapping loses it | `lib/checkers.nix:300`; `(t.struct "s" { a = t.optionalAttr t.int; }).verify { }` ⇒ `null`, but `(t.struct "s2" { a = t.option (t.optionalAttr t.int); }).verify { }` ⇒ `"in struct 's2': missing member 'a'"`. Positive control, plain member: `(t.struct "s3" { a = t.int; }).verify { }` ⇒ `"in struct 's3': missing member 'a'"`. Test: `test-optional-member-absent-ok` (`ci/tests/types-struct.nix`) |
...
| `union` / `intersection` / `tuple` / `struct` assert their argument shape at CONSTRUCTION, before any value is seen | `lib/checkers.nix:207,217,235,281`; `(t.union t.int).verify 1` threw |
...
- **Palmer-style intensional identity, name-only** — `lib/default.nix:43-48` and `lib/checkers.nix:14-16`: two checkers denote the same type iff their names agree, with `__id` the sha256 content address of the name. Both call the name-only limit a deliberate ceiling — no closure or structural comparison. The repo names no fuller reference for "Palmer-style".
- **The `id_hash` discipline of gen-schema** — `README.md:165-167` and `lib/default.nix:45-46` state `__id` uses the same hashing discipline. Cross-repo citation.
